...
net.Receive( "LShop_SendMessage", function( )
	Derma_Message( net.ReadString( ), LShop.lang.GetValue( "LShop_Notice_Text" ), LShop.lang.GetValue( "LShop_Yes" ) )
end )
...
function LShop.cl.MainShop( )
	...
	local function createHooks( )
		hook.Add( "CalcView", "LAdmin.main.SchematicView", function( ply, pos, angles, fov )
			if ( !IsValid( ply ) or !LShop.Config.ShopMenu_ThirdPersonEnabled ) then return end
		...
		hook.Add( "RenderScreenspaceEffects", "LAdmin.main.SchematicView_Func2", function( )
			local color = { }
		...
		hook.Add( "ShouldDrawLocalPlayer", "LAdmin.main.SchematicView_Func", function( )
			if ( !LShop.Config.ShopMenu_ThirdPersonEnabled ) then return false end
			return true
		end )
	end
	
	local function closeFunction( )
		if ( !IsValid( LShop_MainShopPanel ) ) then return end
	...
	LShop_MainShopPanel.Think = function()
		if ( IsValid( LShop_Menu01Panel ) ) then LShop_MainShopPanel:MoveToBack( ) end
		if ( IsValid( LShop_Menu02Panel ) ) then LShop_MainShopPanel:MoveToBack( ) end
		if ( IsValid( LShop_AdminPanel ) ) then LShop_MainShopPanel:MoveToBack( ) end
		if ( IsValid( LShop_PlayerManager ) ) then LShop_MainShopPanel:MoveToBack( ) end
	end
	LShop_MainShopPanel.Paint = function( pnl, w, h )
		...
		if ( day.wday == LShop.Config.DayNumber ) then
			draw.SimpleText( LShop.lang.GetValue( "LShop_Saleday" ), "LShop_SubTitle", w * 0.9, h * 0.05, Color( 0, 0, 0, schematicIntro_alpha ), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
		end
	end
	...
	CloseButton.DoClick = function( pnl )
		...
		closefunc = true
		if ( IsValid( LShop_MainShopPanel.Menu01 ) ) then LShop_MainShopPanel.Menu01:AlphaTo( 0, 0.3, 0 ) end
		if ( IsValid( LShop_MainShopPanel.Menu02 ) ) then LShop_MainShopPanel.Menu02:AlphaTo( 0, 0.3, 0 ) end
		if ( IsValid( LShop_MainShopPanel.Admin ) ) then LShop_MainShopPanel.Admin:AlphaTo( 0, 0.3, 0 ) end
		if ( IsValid( LShop_MainShopPanel.Language ) ) then LShop_MainShopPanel.Language:AlphaTo( 0, 0.3, 0 ) end
		if ( IsValid( pnl ) ) then pnl:AlphaTo( 0, 0.3, 0 ) end
	...
	LShop_MainShopPanel.Menu01:SetText( LShop.lang.GetValue( "LShop_MainButton_Shop" ) )  
	...
	LShop_MainShopPanel.Menu01.Paint = function( pnl, w, h )
	...
	LShop_MainShopPanel.Menu02:SetText( LShop.lang.GetValue( "LShop_MainButton_Inventory" ) )  
	...
	LShop_MainShopPanel.Menu02.Paint = function( pnl, w, h )
	...
	if ( LShop.Config.PermissionCheck( LP ) ) then
		...
		LShop_MainShopPanel.Admin:SetText( LShop.lang.GetValue( "LShop_MainButton_Admin" ) )  
		...
		LShop_MainShopPanel.Admin.Paint = function( pnl, w, h )
	...
	local Bx, By = scrW * 0.03, scrH * 0.95 - 35 / 2

	LShop_MainShopPanel.Language = vgui.Create( "DButton", LShop_MainShopPanel )    
	for k, v in pairs( LShop.lang.tablesCL[ LShop.lang.langconfigCL ] ) do
		if ( k != "LShop_LangFile_Name" ) then continue end
		LShop_MainShopPanel.Language:SetText( LShop.lang.GetValue( "LShop_MainButton_Language" ) .. v )  
	end
	LShop_MainShopPanel.Language:SetFont( "LShop_ButtonText" )
	LShop_MainShopPanel.Language:SetPos( Bx, By )  
	LShop_MainShopPanel.Language:SetColor( Color( 0, 0, 0, 255 ) )
	LShop_MainShopPanel.Language:SetSize( LShop_MainShopPanel_w * 0.15, 35 ) 
	LShop_MainShopPanel.Language:SetAlpha( 0 )
	LShop_MainShopPanel.Language:AlphaTo( 255, 0.3, 0 )
	LShop_MainShopPanel.Language.DoClick = function(  )
		surface.PlaySound( "ui/buttonclick.wav" )
		local Menu = DermaMenu( )
		for k, v in pairs( LShop.lang.tablesCL ) do
			for k1, v1 in pairs( LShop.lang.tablesCL[ k ] ) do
				if ( k1 != "LShop_LangFile_Name" ) then continue end
				Menu:AddOption( v1, function( )
					Derma_Query( LShop.lang.GetValue( "LShop_Warning_1" ) .. " [ " .. v1 .. " ]", LShop.lang.GetValue( "LShop_Warning_Text" ),
						LShop.lang.GetValue( "LShop_Yes" ), function( ) 
							LShop.lang.langconfigCL = k
							LShop_MainShopPanel.Language:SetText( LShop.lang.GetValue( "LShop_MainButton_Language" ) .. v1 )  
							net.Start("LShop_Lang_SaveConfig")
							net.WriteString( LShop.lang.langconfigCL )
							net.SendToServer()
							
							closeFunction( )
							LShop.cl.MainShop()
						end,
						LShop.lang.GetValue( "LShop_No" ), function( ) end
					)
				end )
			end
		end
	...
	LShop_MainShopPanel.Language.Paint = function( pnl, w, h )
		surface.SetDrawColor( 10, 10, 10, 10 )
		surface.DrawRect( 0, 0, w, h )
	end
